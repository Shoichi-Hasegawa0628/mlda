...
class ExtractImgBof():
    ...
    def img_server(self, status, count, observed_img_idx, yolov3_image):
        if status == "learn":
            ct = 1
            for c in range(len(OBJECT_CLASS)):
                for n in range(len(OBJECT_NAME[c])):
                    self.make_codebook(OBJECT_CLASS[c], OBJECT_NAME[c][n], status, None, None)
                    self.make_img_bof(status, OBJECT_CLASS[c], OBJECT_NAME[c][n], None, None)
                    ct += 1
            ...
            if result == 0:
                return 0
            self.make_img_bof(status, None, None, count, observed_img_idx)
    ...
    def make_codebook(self, object_class, object_name, status, count, observed_img_idx):
